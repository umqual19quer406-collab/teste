...
def _sc6_has_ativo(cur) -> bool:
    cur.execute("SELECT CASE WHEN COL_LENGTH('dbo.SC6_ITENS', 'C6_ATIVO') IS NULL THEN 0 ELSE 1 END")
    row = cur.fetchone()
    return bool(row and int(row[0]) == 1)
...
def _sc6_ativo_select(cur) -> str:
    return "C6_ATIVO" if _sc6_has_ativo(cur) else "CAST(1 AS BIT) AS C6_ATIVO"


def _sc6_ativo_filter(cur, alias: str = "") -> str:
    prefix = f"{alias}." if alias else ""
    return f" AND {prefix}C6_ATIVO=1" if _sc6_has_ativo(cur) else ""


def sc6_item_get_tx(cur, item_id: int) -> dict | None:
    ativo_select = _sc6_ativo_select(cur)
    cmv_select = "C6_CMV_UNIT" if _sc6_has_cmv_unit(cur) else "CAST(NULL AS DECIMAL(18,2)) AS C6_CMV_UNIT"
    cur.execute(
        f"""
        SELECT *, {ativo_select}, {cmv_select}
        FROM dbo.SC6_ITENS
        WHERE ID=?
...
def sc6_itens_do_pedido_tx(cur, pedido_id: int) -> list[dict]:
    cmv_select = "C6_CMV_UNIT" if _sc6_has_cmv_unit(cur) else "CAST(NULL AS DECIMAL(18,2)) AS C6_CMV_UNIT"
    ativo_select = _sc6_ativo_select(cur)
    ativo_filter = _sc6_ativo_filter(cur)
    cur.execute(
        f"""
        SELECT ID, C6_PEDIDO_ID, C6_ITEM, C6_PRODUTO, C6_QTD,
               C6_PRECO_UNIT, C6_TOTAL, C6_VALOR, {ativo_select}, {cmv_select}
        FROM dbo.SC6_ITENS
        WHERE C6_PEDIDO_ID=? {ativo_filter}
          AND (D_E_L_E_T_ IS NULL OR D_E_L_E_T_ <> '*')
        ORDER BY ID ASC
        """,
        (int(pedido_id),),
